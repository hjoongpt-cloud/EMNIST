...
def main():
    ...
    for seed in seeds:
        ...
        for ep in range(1, cfg.opt.epochs+1):
            ...
            if debug:
                sampled_norms = []
                sample_prob = cfg.get('debug_sample_prob', 0.05)      # 5% 배치당
                sample_per_batch = cfg.get('debug_sample_per_batch', 64)
                ...
                for imgs, _ in train_loader:
                    if random.random() < sample_prob:
                        imgs = imgs.to(device)
                        with torch.no_grad():
                            pooled, _, _ = model.trunk(imgs)      # (B, D)
                        norms = pooled.norm(dim=1).cpu().numpy()  # (B,)
                        idxs_np = np.random.choice(len(norms),
                                                size=min(sample_per_batch, len(norms)),
                                                replace=False)
                        sampled_norms.extend(norms[idxs_np].tolist())
                    # 너무 많이 돌지 않도록, 첫 20 배치만 샘플링
                    if len(sampled_norms) > sample_per_batch * 20:
                        break            
            ...
            if debug and sampled_norms:
                # 1) 히스토그램 + 퍼센타일 라인 + heatmap
                counts, bins = np.histogram(sampled_norms, bins=50)
                centers = 0.5 * (bins[:-1] + bins[1:])
                pcts = [10,20,30,40,50,60,70,80,90]
                pct_vals = np.percentile(sampled_norms, pcts)
 
                fig, (ax_h, ax_hm) = plt.subplots(
                    2, 1,
                    figsize=(6, 5),
                ...
                # (a) Histogram
                width = bins[1] - bins[0]
                ax_h.bar(centers, counts, width=width, color='C0', edgecolor='k')
                for pt, val in zip(pcts, pct_vals):
                    ax_h.axvline(val, color='r', linestyle='--', alpha=0.7)
                    ax_h.text(val, ax_h.get_ylim()[1]*0.9, f'{pt}%', 
                              rotation=90, va='top', ha='right', color='r')
                ax_h.set_title(f'Epoch {ep:02d} pooled-feature norm distribution')
                ax_h.set_xlabel('L2 norm of pooled feature')
                ax_h.set_ylabel('Frequency')
 
                # (b) Simple heatmap (정규화된 counts)
                norm_counts = counts / counts.max()
                ax_hm.imshow([norm_counts], aspect='auto', cmap='hot',
                             extent=[bins[0], bins[-1], 0, 1])
                ax_hm.axis('off')
 
                # 저장
                plt.tight_layout()
                plt.savefig(os.path.join(run_dir, f'pool_norms_ep{ep:03d}.png'))
                plt.close(fig)
        # Eval & probe
